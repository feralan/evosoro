...
def JustSimulateDontEvaluate(sim, env, pop, print_log, save_vxa_every, run_directory, run_name, max_eval_time=60,
    ...
    print_log.message("Launched {0} voxelyze calls, out of {1} individuals".format(num_imported_this_gen, len(pop)))

    num_imports_finished = 0
    all_done = False
    already_analyzed_ids = []
    redo_attempts = 1
        ...
        if ls_check:
            ...
            if "softbotsOutput--id_" in ls_check:
                ...
                if this_id in already_analyzed_ids:
                    ...
                    print_log.message("{0} fit = {1} ({2} / {3})".format(ls_check, objective_values_dict[0],
                                                                         num_imports_finished,
                                                                         num_imported_this_gen))
